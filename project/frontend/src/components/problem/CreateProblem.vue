...
const problem = ref({
  ...
  samples: [{id: null, input: '', output: ''}],
  tests: [{id: null, input: '', output: ''}],
  tags: []
});
...
const removeSample = (index: number) => {
  if (problem.value.samples.length > 1) {
...
const removeTest = (index: number) => {
  if (problem.value.tests.length > 1) {
    problem.value.tests.splice(index, 1);
  } else {
...
const submitProblem = () => {
  client.problems.create({
    title: problem.value.title,
    description: problem.value.description,
    input: problem.value.input,
    output: problem.value.output,
    memoryLimitBytes: problem.value.memoryLimitBytes,
    timeLimitMilliseconds: problem.value.timeLimitMilliseconds,
...
const handleTagsUpdate = (params: TagsDto) => {
  problem.value.tags = params.tags.map(tag => tag.id);
};
</script>
    ...
    <form @submit.prevent="submitProblem">
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <input
              type="number"
              v-model.number="problem.timeLimitMilliseconds"
              min="100"
              step="100"
              required
          >
        ...
        <div class="form-group">
          ...
          <input
              type="number"
              v-model.number="problem.memoryLimitBytes"
              min="1"
              step="1"
              required
          >
      ...
      <div class="form-section">
        <h3>Tags</h3>
        <div class="form-group">
          <TagPicker :tags="client.tags.cache" :selected-tags="problem.tags.map(id => client.tags.getById(id))"
                     @tags-update="handleTagsUpdate"/>
        </div>
      </div>

      <div class="form-actions">
        <button type="submit" class="btn-submit">Create Problem</button>
      </div>
    </form>
  </div>
</template>
