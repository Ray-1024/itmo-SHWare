...
const compiledMarkdown = computed(() => {
  return DOMPurify.sanitize(marked(article.value.article));
});
...
const insertText = (textToInsert: string) => {
  ...
  const selectedText = article.value.article.substring(startPos, endPos);
  ...
  article.value.article =
      article.value.article.substring(0, startPos) +
      wrappedText +
      article.value.article.substring(endPos);
...
async function handleTagsUpdate(params: TagsDto) {
  article.value.tags = params.tags.map(t => t.id);
}
...
<template>
  <div class="markdown-editor">
    <div class="editor-header">
      ...
      <div class="toolbar">Title:<input v-model="article.title" class="article-title"></div>
      <div class="toolbar">
    ...
    <div class="editor-footer">
      <div class="toolbar">
        <TagPicker :tags="client.tags.cache" @tags-update="handleTagsUpdate"
                   :selected-tags="article.tags.map(id => client.tags.getById(id))"/>
      </div>
      <button @click="saveArticle" class="save-button">Create Article</button>
